...
# hybdir <- "/media/hermione/AGO1_CLASH/Process3/SRR959751_chimera.tsv"
# rnaplexname <- "/media/hermione/AGO1_CLASH/vienna/SRR959751.rnaplex"
# outname <- "/media/hermione/AGO1_CLASH/vienna/SRR959751"
...
rnaplex <- read_tsv(rnaplexname, 
                  col_names = "data") %>% 
  mutate(index = case_when(str_detect(data,"microRNA") ~ "miRNA2",
                           str_detect(data,">\\d+_\\d+") ~ "readname",
                           TRUE ~ "match"),
         counter = if_else(index =="miRNA2", 1,0) %>% cumsum()) %>% 
...
rnaplex2 <- rnaplex %>% mutate(match = match %>% str_replace(" \\(", "  \\(") %>% str_replace_all("   ", "  ") %>% str_replace_all("\\) 1", "\\)  1") %>% 
  ...
  mutate(miRNA2 = miRNA2 %>% str_remove(">"),readname = readname %>% str_remove(">"),
         dG = dG_raw %>% str_extract("\\(.+\\)") %>%  str_remove_all("\\(|\\)") %>% as.double()) 
rnaplex3 <- hybtest %>% mutate(miRNA2 = miRNA) %>% 
  left_join(rnaplex2,by = c("miRNA2","readname")) %>% 
  mutate(targetlen = targetlen %>% as.integer(),
         milen = milen %>% as.integer()) %>% 
  separate(miRNA, sep = "_", into = c("miRNA","dis2","miRNAname", "dis3")) %>% 
  separate(target2, sep = "_", into = c("target","dis4","targetname","targettype")) %>% 
  mutate(miRNAmatch = paste0(strrep(".", p3_start-1), p3bind, strrep(".", milen - p3_end)) %>% str_replace_all("\\)", "\\("),
         targetmatch = paste0(strrep(".", p5_start-1), p5bind, strrep(".", targetlen - p5_end)) %>% str_replace_all("\\(", "\\)")) %>% 
  select(-contains("p3"), -contains("p5"), -contains("dis"), -miRNA2, -dG_raw) %>% rename(index = counter) %>% 
  mutate(miRNAmatch = if_else(!(str_detect(miRNAmatch, "\\(")), strrep(".", milen),miRNAmatch),
         targetmatch = if_else(!(str_detect(miRNAmatch, "\\(")), strrep(".", targetlen),targetmatch))
rnaplex3 %>% write_tsv(paste0(outname,"_vienna.tsv"))
#vienna
vienna_struct <- hybtest %>% mutate(miRNA2 = miRNA) %>% 
  left_join(rnaplex2,by = c("miRNA2","readname")) %>% 
  separate(miRNA, sep = "_", into = c("miRNA","dis2","miRNAname", "dis3")) %>% 
  mutate(targetlen = targetlen %>% as.integer(),
         milen = milen %>% as.integer(),
         miRNAmatch = paste0(strrep(".", p3_start-1), p3bind, strrep(".", milen - p3_end)) %>% str_replace_all("\\)", "\\("),
         targetmatch = paste0(strrep(".", p5_start-1), p5bind, strrep(".", targetlen - p5_end)) %>% str_replace_all("\\(", "\\)")) %>% 
  filter(str_detect(miRNAmatch, "\\(")) %>% 
  mutate(contact = paste0(".",miRNAmatch,".",  targetmatch,"."),
         seq = paste0("A",miRNAseq, "A",targetseq, "A"))
