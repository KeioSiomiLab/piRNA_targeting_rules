# compare AGO1_CLASH and PRG1 CLASH
.libPaths("~/R/x86_64-pc-linux-gnu-library/4.2/")



suppressMessages(suppressWarnings(require(RRNA)))
...
filedir2 <- "/media/hermione/AGO1_CLASH/"
filedir3 <- "/media/hermione/PRG1_CLASH/"
filedir4 <- "/media/hermione/cCLIP/"
filedir6 <- "/media/hermione/SMEDWI3/"
outdir <- "/media/pericles/CLASH/"
Vienna_sle <- read_csv(paste0(outdir, "vienna1fig/clash_colapse.csv"), col_types = cols(dG = col_double())) # %>% filter(is.finite(dG))
Vienna_sle_AGO <- read_csv(paste0(filedir2, "vienna1fig/clash_colapse.csv"), col_types = cols(dG = col_double())) %>% #  filter(is.finite(dG)) %>%
  rename(piRNAmatch = miRNAmatch,
         piRNAseq = miRNAseq)
Vienna_sle_PRG1 <- read_csv(paste0(filedir3, "vienna1fig/clash_colapse.csv"), col_types = cols(dG = col_double())) # %>% filter(is.finite(dG))
Vienna_sle_Aub <- read_csv(paste0(filedir4, "vienna1fig/clash_colapse.csv"), col_types = cols(dG = col_double())) # %>% filter(is.finite(dG))
Vienna_sle_SMEDWI3 <- read_csv(paste0(filedir6, "vienna1fig/clash_colapse.csv"), col_types = cols(dG = col_double())) # %>% filter(is.finite(dG))
...
cal_struct <- function(dat) {
  ...
  if (dim(res)[1]==0) {
    ...
    res2 <- res %>% mutate(before = map2(total,total3, function(x,y) {x[1:y] %>% length()}),
      ...
      unnest_legacy() %>%
      mutate(anno = if_else(before==2 & after == 2, "mismatch",
...
datastruct <- function(data,out){
  table_RNA <- data %>% mutate(contact = paste0(piRNAmatch,".", targetmatch),
                               seq = paste0(piRNAseq, "A",targetseq)) 
  table_RNA2 <- table_RNA %>% select(contact, seq) %>% distinct() %>% 
    rowwise() %>%
    mutate(ct = makeCt(struct = contact, seq = seq) %>% ct2coord_up() %>% cal_struct() %>% .$anno %>% paste0(collapse = "@"))
  tableRNA3 <- table_RNA %>% left_join(table_RNA2, by = c("contact","seq")) %>%
    select(-contact, -seq) %>% 
    mutate(ct = if_else(!str_detect(piRNAmatch, "\\("), "else",ct))
  tableRNA3 %>% write_tsv(paste0(outdir, "compare/", out, "_struct.csv"))
}
Vienna_sle %>% datastruct(out = "Piwi")
Vienna_sle_AGO %>% datastruct(out = "AGO")
Vienna_sle_PRG1 %>% datastruct(out = "PRG1")
Vienna_sle_Aub %>% datastruct(out = "Aub")
Vienna_sle_SMEDWI3 %>% datastruct(out = "SMEDWI3")
