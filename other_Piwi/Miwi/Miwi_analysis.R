...
RNAtype <- read_tsv("/media/hermione/otherdata_compare/Miwi/database/Mm_gene_annotation.txt") 

#TEtype <- read_tsv("/media/hermione/otherdata_compare/Miwi/database/MmTE_anno.tsv")

outdir <- "/media/hermione/otherdata_compare/Miwi/figures/"
readdir <- "/media/hermione/otherdata_compare/Miwi/"
...
read_info_RNA <- read_tsv(paste0(readdir,"read_info.tsv")) %>% 
  mutate(type = file %>% str_remove("_trim.fastq") %>% str_remove("_trimed3.fastq") %>% str_remove(".fastq") %>%
...
All_edgeR <- bind_rows(TE_count %>% mutate(RNAtype = "TE"), mRNA_count %>% mutate(RNAtype = "mRNA")) %>% 
  ...
  mutate(ctrl = SRR363965, KD = SRR363967) %>% select(-contains("SRR")) %>% 
...
table2 <- table %>% left_join(All_count %>% rename(name = gene_name) %>% select(name, RNAtype), by = "name") %>% 
  ...
  left_join(RNAtype %>% rename(name = gene_id, genesymbol = gene_symbol) %>% select(name,genesymbol), by = "name") %>% 
 # left_join(TEtype %>% rename(TEsymbol = name) %>% rename(name = TEname)%>%  select(name, TEsymbol), by = "name") %>% 
  mutate(genesymbol = if_else(is.na(genesymbol), name,genesymbol))
#edit here#####################################################################################################
changed_gene_list <- table2 %>% filter(str_detect(name, "L1MdA|L1MdGf|L1MdTf")) %>% 
  filter(logFC > 1) %>% filter(RNAtype =="TE") %>% pull(genesymbol)
###############################################################################################################
#change_order <- c("negative","positive","target RNA")
change_order <- c("negative","target RNA")
table3 <- table2 %>%
...
table3 %>%  
  ...
  geom_point(size = 1.4,alpha = 0.6)+ scale_color_manual(values = c( "#808080","blue")) +
#  geom_text_repel(data = subset(table3, type == "target RNA"),
#                  aes(label = genesymbol), color = "blue4", size = 2,max.overlaps = Inf,
...
# exon piRNA filter
exon_annotation <- read_tsv(paste0(readdir ,"rnaplex/exon_overlap.tsv"),
                            col_names = c("targetname","start","end","readname","dis1","dis4","dis5","genesymbol","dis2","genestrand"))

chr_other_genes <- read_tsv("/media/hermione/otherdata_compare/Miwi/database/gene.bed",
                            col_names = c("chr","start","end","genename","score","strand")) %>% 
  select(chr,genename) %>% filter(!(str_detect(chr, "chr")))

# add annotation
gene_annotation <- read_tsv(paste0(readdir ,"rnaplex/","gene_overlap.tsv"),
                        col_names = c("targetname","start","end","readname","dis1","dis4","dis5","genesymbol","dis2","genestrand")) %>% 
  mutate(sstart2 = start,send2 = end-1) %>% 
  select(-contains("dis"),-start,-end)

#test
#piRNA_CPM <- read_tsv(paste0("/media/hermione/otherdata_compare/Siwi/","piRNA_CPM.tsv"))
#remove repeat
...
#  anti_join(chr_other_genes %>% rename(targetname = genename), by = "targetname") %>% 
  left_join(gene_annotation , by = c("targetname","readname","sstart2","send2")) %>% 
 # left_join(TEtype %>% rename(TEsymbol = name) %>% rename(targetname = TEname)%>%  select(targetname, TEsymbol), by = "targetname") %>% 
  mutate(genesymbol = if_else(is.na(genesymbol) & pairtype == "TE", targetname,genesymbol)) %>% 
  mutate(genestrand = if_else(pairtype == "TE", "+", genestrand)) %>% 
  filter(!is.na(genesymbol)) %>% 
  filter((strand == "antisense" & genestrand == "+") | (strand == "sense" & genestrand == "-")) %>% 
#  left_join(RNAtype %>% rename(RNAtype = type,targetname = name), by = "targetname") %>% 
  mutate(RNAtype = pairtype,
 ...
 # filter(RNAtype != "ncRNA") %>% # filter(remover != "remove") %>%  filter(remover2 != "remove") %>% 
  ...
  left_join(exon_annotation %>% select(readname, genesymbol,dis2) %>% distinct() %>% filter(dis2 == "protein_coding"), by = c("readname","genesymbol")) %>% 
  filter(!(pairtype == "gene" & is.na(dis2))) %>% select(-dis2) %>% 
  left_join(RNAtype %>% select(-gene_name) %>% rename(gene_name = gene_symbol, genesymbol = gene_id) %>% select(genesymbol,gene_name), by = "genesymbol") %>% 
  mutate(gene_name = if_else(is.na(gene_name),genesymbol,gene_name))
Vienna1_model %>% write_csv(paste0(outdir, "piRNA_model.tsv"))

# ROC
Vienna2_all <- Vienna1_model %>% filter(RNAtype %in% c("TE","gene")) %>%  #filter(type3 !="rmst") %>%
  mutate(ki = exp((-dG * 4.184 * 1000)/(8.3145 * 299.15)),
         kiE  = ki * abattomol) %>% 
  group_by( RNAtype,genesymbol,gene_name) %>% 
  summarise(psi = sum(kiE),
            omega = sum (abattomol),meanpilen = mean(pilen),
            meandG = mean(dG), pinum = n()) %>% ungroup() %>% mutate(targetname = genesymbol) %>% 
  ...
  select(targetname, gene_name,RNAtype, genesymbol,Teyes,psi,omega,phi,chi,tau,ctrl_CPM,KD_CPM, meandG,pinum,meanpilen,logFC) %>% filter(is.finite(tau))
...
Vienna2_all %>% mutate(Teyes = Teyes %>% factor(levels = change_order)) %>% 
  ...
  ggrepel::geom_text_repel(data = subset(Vienna2_all %>% filter(genesymbol %in% c("Dmel-GATE","Dmel-McClintock","Dmel-3S18","Dmel-diver","Dmel-copia","Dmel-gypsy7"))),
  ...
  ggrepel::geom_text_repel(data = subset(Vienna2_all %>% filter(genesymbol %in% c("Dmel-transib2","Dmel-mdg1","Dmel-ZAM","Dmel-gtwin"))),
  ...
  ggsave(paste0(outdir,"TE_piCPM_rate2.png"), width =3.8, height =4.2, dpi = 300)+
  ggsave(paste0(outdir,"TE_piCPM_rate2.pdf"), width =3.8, height =4.2)

change_order2 <- c("target RNA","independent", "mRNA")
Vienna2_all %>% mutate(Teyes = if_else(Teyes=="independent" & RNAtype =="gene", "mRNA", Teyes) %>% factor(levels = change_order2)) %>% 
  ggplot(aes(x = omega, y =logFC, color = Teyes))+
  geom_point(alpha = 0.8)+ scale_x_log10()+ theme_minimal()+
  ggrepel::geom_text_repel(data = subset(Vienna2_all %>% filter(genesymbol %in% c("L1MdGf_II","L1MdA_VI","L1MdTf_III"))),
                           aes(label =genesymbol), size = 2.7, color = "magenta4",  point.padding = 0.1,max.overlaps =Inf,
                           box.padding = unit(1.2, "lines"), min.segment.length = 0)+
  ggrepel::geom_text_repel(data = subset(Vienna2_all %>% filter(genesymbol %in% c("MERVL","IAPEZI","B1"))),
                           aes(label =genesymbol), size = 2.7, color = "black",  point.padding = 0.1,max.overlaps =Inf,nudge_x = 0.5, nudge_y =-0.5,
                           box.padding = unit(1.2, "lines"), min.segment.length = 0)+
