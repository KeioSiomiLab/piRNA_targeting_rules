...
outdir <- "/media/hermione/otherdata_compare/Aub_embryo/figures/"
readdir <- "/media/hermione/otherdata_compare/Aub_embryo/"
#######################################################################################################
# RNA-seq analysis
#######################################################################################################
#mRNA
...
All_edgeR <- bind_rows(TE_count %>% mutate(RNAtype = "TE"), mRNA_count %>% mutate(RNAtype = "mRNA")) %>% 
  ...
  mutate(ctrl1 = SRR3051356, ctrl2 = SRR3051357,KD1 = SRR3051361,KD2 = SRR3051362) %>% select(-contains("SRR")) %>% 
...
#edit here#####################################################################################################
changed_gene_list <- c("Dmel-3S18","Dmel-diver","Dsim-HMS-Beagle","Dmel-Burdock","Dmel-copia","Dmel-Het-A","Dmel-flea",
                       "Dmel-TART-A","Dmel-TART-B","Dmel-Max-element","Dmel-mdg3","Dmel-Transpac",
                       "Dmel-412","Dmel-TAHRE","Dmel-invader3","Dmel-blood")
###############################################################################################################
change_order <- c("negative","positive","target RNA")
table3 <- table2 %>%
  mutate(ctrl_CPM = 1000000 * (ctrl1+ctrl2)/ sum(ctrl1+ctrl2),
         KD_CPM = 1000000 * (KD1+KD2)/ sum(KD1+KD2),
...
Vienna2_all <- Vienna1_model %>% filter(RNAtype %in% c("TE","mRNA")) %>%  #filter(type3 !="rmst") %>%
  ...
  select(targetname, RNAtype, genesymbol,Teyes,psi,omega,phi,chi,tau,ctrl_CPM,KD_CPM, meandG,pinum,meanpilen,logFC) %>% filter(is.finite(tau))
...
Vienna2_all %>% mutate(Teyes = Teyes %>% factor(levels = change_order)) %>% 
  ...
  ggrepel::geom_text_repel(data = subset(Vienna2_all %>% filter(genesymbol %in% c("Dmel-Max-element","Dmel-Burdock","Dmel-3S18","Dmel-diver","Dmel-copia","Dmel-Transpac"))),
  ...
  ggrepel::geom_text_repel(data = subset(Vienna2_all %>% filter(genesymbol %in% c("Dmel-gypsy","Dmel-mdg1","Dmel-ZAM","Dmel-gtwin"))),
                           aes(label =genesymbol), size = 2.7, color = "black",  point.padding = 0.1,max.overlaps =Inf,nudge_x = 0.5, nudge_y =-0.5,
                           box.padding = unit(1.2, "lines"), min.segment.length = 0)+
  labs(x = "piRNA expression (CPM)\n(perfect and near-perfect match)",y = "piRNA/mRNA") +
  theme(legend.title = element_blank(),legend.position = "bottom")+
  ggsave(paste0(outdir,"TE_piCPM_rate2.png"), width =4, height =3.5, dpi = 300)+
  ggsave(paste0(outdir,"TE_piCPM_rate2.pdf"), width =4, height =3.5)
