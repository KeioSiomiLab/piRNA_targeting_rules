...
outname <- "/media/hermione/piRNAmodel/rnaplex/"
#######################################################################################################
...
piRNA_model <- read_tsv("/media/hermione/piRNAmodel/blastout/piRNA.txt",
  ...
  mutate(type = case_when((pilen == length & gapopen == 0) & mismatch==0 ~ "perfect match",
                          ...
                          TRUE ~ "not functional"))
tes <- piRNA_model %>% 
  group_by(sseqid,type) %>% count() %>% ungroup() 
...
piRNA_model2 %>% write_tsv(paste0(outname, "model_RNAplex.tsv"))
...
piRNA_model3 <- piRNA_model2 %>% mutate(index = row_number(),piRNA = paste0(">",piRNAname), readname = paste0(">",readname)) %>% 
  ...
  select(fasta)
piRNA_model3 %>% write_tsv(paste0(outname, "model_RNAplex.fasta"), col_names = FALSE)
...
piRNA_model_gene <- read_tsv("/media/hermione/piRNAmodel/blastout/gene.txt",
                        col_names = c("qseqid", "sseqid", "pident", "length", "mismatch", "gapopen",
                                      "qstart", "qend", "sstart", "send", "evalue", "bitscore")) %>% 
  mutate(qseqid = qseqid %>% str_extract("piRNA\\d+")) %>%
  left_join(piRNA_seq %>% rename(qseqid = name), by = "qseqid") %>% 
