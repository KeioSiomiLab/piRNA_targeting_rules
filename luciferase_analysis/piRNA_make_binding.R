...
# hybdir <- "/media/hermione/piRNA_luc/hyb_temp/mdg1_anti_11_13.tsv"
# rnaplexname <- "/media/hermione/piRNA_luc/rnaplex/mdg1_anti_11_13.rnaplex"
# outname <- "/media/hermione/piRNA_luc/binding/mdg1_anti_11_13"
...
rnaplex <- read_tsv(rnaplexname,
  ...
  mutate(index = case_when(str_detect(data,"piRNA") ~ "piRNA",
                           str_detect(data,">mdg1|>pi_RNA|>flam") ~ "target",
                           TRUE ~ "match"),
...
rnaplex2 <- rnaplex %>% mutate(match = match %>% str_replace(" \\(", "  \\(") %>% str_replace_all("   ", "  ") %>% str_replace_all("\\) 1", "\\)  1") %>%
  ...
  mutate(piRNA = piRNA %>% str_remove(">"), target = target %>%  str_remove(">"),
         dG = dG_raw %>% str_extract("\\(.+\\)") %>%  str_remove_all("\\(|\\)") %>% as.double())


rnaplex3 <- hybtest %>%
  rename(piRNA = qseqid,target = vector, piRNAseq = seq,targetseq = seq2) %>%
  mutate(pilen = str_length(piRNAseq), targetlen = str_length(targetseq)) %>%
  left_join(rnaplex2,by = c("piRNA","target")) %>%
...
rnaplex3 %>%
  write_tsv(paste0(outname,"_binding.tsv"))
